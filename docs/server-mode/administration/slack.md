...
Configuration des notifications Slack pour les contributions d'offres d'énergie.
...
Dans le fichier `apps/api/.env.docker` :
...
- Vérifiez que `SLACK_WEBHOOK_URL` est bien défini dans `.env.docker`
- Vérifiez que le fichier `.env.docker` est bien monté dans le container Docker
...
2. Vérifier les logs :
   ...
   ```bash
   docker compose logs backend | tail -100 | grep -i slack
   ```
